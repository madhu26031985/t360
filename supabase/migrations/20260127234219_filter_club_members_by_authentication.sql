/*
  # Filter Club Members by Authentication Status

  1. Changes
    - Update get_club_members_report to only show authenticated members
    - Add WHERE clause filter for is_authenticated = true
  
  2. Purpose
    - Only display members who are authenticated/active in the club
    - Members with is_authenticated = false should not appear in reports
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE acur.club_id = p_club_id 
    AND acur.user_id = auth.uid()
    AND acur.is_authenticated = true
  ) THEN
    RAISE EXCEPTION 'Access denied: You are not a member of this club';
  END IF;

  -- Return member data (only authenticated members)
  RETURN QUERY
  SELECT
    u.id,
    u.full_name,
  ...
  GROUP BY u.id, u.full_name, u.email, u.phone_number, u.avatar_url, u."About", u.facebook_url, u.linkedin_url, u.instagram_url, u.twitter_url, u.youtube_url, cur.role, cur.is_authenticated
